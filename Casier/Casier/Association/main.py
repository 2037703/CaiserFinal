...
# Importer le popup_casier

from popup_casier import *
...
class fenetrePrincipale(QtWidgets.QMainWindow, interfaceMain.Ui_MainWindow):
    """
    ...
    # Bouton ouvrir casier

    @pyqtSlot()
    def on_pushButton_cas_open_clicked(self):
        """
        Méthode du bouton casier pour ouvrir la fenêtre casier
        ...
        # Entrée de données des attributs de la classe étudiant
        etud2.NomEtud = self.lineEdit_nom.text().capitalize()
        etud2.NumEtud = self.lineEdit_numero.text()
        trouve = False
        for e in ls_Etudiants:
            if etud2.NumEtud == e.NumEtud:
                trouve = True
                # Instanciation une boite de dialogue PopUpCasier
                boiteCasier = PopUpCasier(e, self.textBrowser_afficher)
